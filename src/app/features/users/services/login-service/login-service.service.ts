...
import { CookieService } from 'ngx-cookie-service';
import { isPlatformBrowser } from '@angular/common';
import { AuthService } from '../Session/auth.service'; // Use full AuthService
...
export class LoginService {
  ...
  constructor(
    ...
    this.refreshFromStorage(); // Hydrate state at init
  }
  ...
  refreshFromStorage(): void {
    ...
    if (!token || !decoded || this.authService.isTokenExpired(token)) {
      console.log('[LoginService] JWT expired or invalid, clearing data...');
      this.clearData();
      return;
    }
  ...
  setLoginType(loginType: string) {
    ...
    const expires = this.authService.getTokenExpiry();
    if (expires) {
      this.cookieService.set('loginType', loginType, {
        expires,
        path: '/',
  ...
  clearData() {
    ...
    console.log('[LoginService] Cleared login data');
  }
}
